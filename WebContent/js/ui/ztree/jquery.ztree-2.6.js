	...
	function appendTreeNodes(setting, level, treeNodes, parentNode) {
		...
		for (var i = 0, l = treeNodes.length; i < l; i++) {
			...
			html.push("<a id='", node.tId, IDMark_A, "' treeNode", IDMark_A," onclick=\"", (node.click || ''),
				...
				"' title='", node.title ,"'>",node[setting.nameCol].replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'),"</span></a><ul id='", node.tId, IDMark_Ul, "' class='", makeUlLineClass(setting, node), "' style='display:", (node.open ? "block": "none"),"'>");
