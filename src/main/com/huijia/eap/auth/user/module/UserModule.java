...
public class UserModule {
	...
	public void prepare(HttpServletRequest request, @Param("userId") long userId, 
	...
	@Ok("json")
	@Chain("validate")
	public void editAjax(@Param("..") User user){
		userService.update(user);
	}
	...
	public boolean saveValidName(@Param("name") String name,
		...
		if (operation == null || operation.trim().length() == 0 || name == null || name.trim().length() == 0)
			return false;
		
		User user = userService.fetchByName(name);
		switch (operation) {
			case OPERATION_EDIT:
				return user == null || userId != null && userId.matches("^\\d+$") && user.getUserId() == Long.parseLong(userId);
			case OPERATION_ADD:
			default:
				return user == null;
		}
