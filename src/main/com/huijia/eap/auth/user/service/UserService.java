...
public class UserService extends TblIdsEntityService<User> {
	...
	public void update(User user) {
		if ("".equals(user.getPassword())|| User.PASSWORD_FADE.equals(user.getPassword())) {
	...
	public User checkUser(String username, String pwd, boolean encrypted) {
		...
		return ((UserDao) this.dao()).checkUser(username, DigestUtil.encodeSHA(pwd));
	}
	...
	public Pager<User> paging(Condition condition, Pager<User> pager) {
		List<User> users = query(condition, this.dao().createPager(pager.getPage(), pager.getPageSize()));
	...
	private Pattern where = Pattern.compile("^[ \t]*WHERE[ \t]+", Pattern.CASE_INSENSITIVE);
	...
	private Condition mergeCondition(Condition condition) {
		...
		if (con == null) {
			newCondition = Cnd.where("type", ">", 0).and("userId", ">", 1);
				...
				newCondition = Cnd.wrap(con.replaceFirst("^[ \t]*WHERE[ \t]+", " WHERE type>0 AND userId>1 AND "));
		} else {
