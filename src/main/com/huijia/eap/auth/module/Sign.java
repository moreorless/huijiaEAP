...
public class Sign {
	...
	public View signin(@Param("username") String username,
			@Param("password") String password,
			@Param("map:infos") Map<String, Object> infos,
			@Param("lang") String lang, HttpServletRequest request, Ioc ioc,
			@Param("redirect") String redirect, HttpServletResponse response) {

		// 兼容性处理，如果map:infos和username/password同时存在则以map:infos为准
		if (!infos.containsKey(IAuthenticator.INFO_KEY_USERNAME)
				&& !infos.containsKey(IAuthenticator.INFO_KEY_PASSWORD)) {
		...
		if (currentUser != null
				&& currentUser.getName().equals(
						infos.get(IAuthenticator.INFO_KEY_USERNAME))) {
		...
		// 首先检查是否第一次注册
		UserTempService userTempService = ioc.get(UserTempService.class,
				"userTempService");
		if (userTempService.checkUserTemp(username, password, false) != null) {
			UserTemp userTemp = userTempService.fetchByCode(username);
			User user = new User();
			user.setCode(userTemp.getCode());
			user.setName(userTemp.getCode());
		...
		UserCheckBean uc = GlobalConfig.getContextValueAs(UserCheckBean.class,
		...
		if (authen) {
			User user = new User();
			if (service.fetchByName((String) infos
					.get(IAuthenticator.INFO_KEY_USERNAME)) != null)
				user = service.fetchByName((String) infos
						.get(IAuthenticator.INFO_KEY_USERNAME));
			else if (service.fetchByCode((String) infos
					.get(IAuthenticator.INFO_KEY_USERNAME)) != null)
				user = service.fetchByCode((String) infos
						.get(IAuthenticator.INFO_KEY_USERNAME));
			else if (service.fetchByMobile((String) infos
					.get(IAuthenticator.INFO_KEY_USERNAME)) != null)
				user = service.fetchByMobile((String) infos
						.get(IAuthenticator.INFO_KEY_USERNAME));
			else {

				EC error = new EC("auth.signin.errors.signfailed", new Bundle(
						"auth"));
				throw ExceptionWrapper.wrapError(error);
			}

			authenticator.ok(user, infos);
			if (service.isUserValid(user) == false) {
				EC error = new EC("auth.signin.errors.invalid", new Bundle(
						"auth"));
				throw ExceptionWrapper.wrapError(error);
			}
			if (user.getLockedAt() > 0
					&& System.currentTimeMillis() - user.getLockedAt() <= uc
							.getTimeLock() * 60000) {
				EC error = new EC("auth.signin.errors.locked", new Bundle(
						"auth"), user.getName());
				throw ExceptionWrapper.wrapError(error);
			} else {
				...
				if (uc.getSessionLock() > 0) {
					int online = 0;
					Map<String, Map<String, User>> onlineUsers = (Map<String, Map<String, User>>) request
							.getServletContext().getAttribute(
									UserCounterListener.USER_KEY);
					if (onlineUsers != null) {
						Collection<Map<String, User>> valueSet = onlineUsers
								.values();
						synchronized (valueSet) {
							Iterator<Map<String, User>> ite = valueSet
									.iterator();
							while (ite.hasNext()) {
								...
								if (hostUsers != null) {
									...
									synchronized (users) {
										Iterator<User> userIte = users
												.iterator();
										while (userIte.hasNext()) {
											User ou = userIte.next();
											if (user.getUserId() == ou
													.getUserId())
												online++;
										}
									}
					...
					if (online >= uc.getSessionLock()) {
						EC error = new EC("auth.signin.errors.onlinelimit",
								new Bundle("auth"), uc.getSessionLock());
						throw ExceptionWrapper.wrapError(error);
					}
				}

				if (reloginAttributes != null) {
					for (Map.Entry<String, Object> entry : reloginAttributes
							.entrySet())
				...
				LocalizationUtil.setSessionLocale(request, response, session,
						lang);

				// 登录成功后，加载导航配置
				session.setAttribute(NavigatorHelper.NAV_SESSION_KEY,
						NavigatorHelper.loadNavigator());

				login = true;
				updateLoginErrorCounts(request.getServletContext(), username,
						true);
			...
			int cnt = updateLoginErrorCounts(request.getServletContext(),
					username, false);
			if (cnt >= uc.getCountLock()) {

				User loginUser = service.fetchByName(username);
				if (loginUser != null)
					updateLockedAt(service, loginUser,
		...
		if (login) {
			if (redirect != null && redirect.trim().length() > 0) {
				...
				if (path.startsWith("/")
						&& (path.endsWith(".jsp") || path.indexOf(".") < 0))
	...
	private static synchronized int updateLoginErrorCounts(ServletContext sc,
			String username, boolean clear) {
		Map<String, Integer> counts = (Map<String, Integer>) sc
				.getAttribute(SC_KEY_LOGIN_ERROR_CNTS);
		if (counts == null) {
	...
	private static synchronized void updateLockedAt(UserService service,
			User user, long lockedAt) {
		user.setPassword(User.PASSWORD_FADE);
		user.setLockedAt(lockedAt);
		service.update(user);
	}

	public static final synchronized void unlockUser(ServletContext sc,
			String username) {
		updateLoginErrorCounts(sc, username, true);
		UserService service = Mvcs.getIoc().get(UserService.class,
				"userService");
